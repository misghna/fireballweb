...
public class ApiController implements EWrapper,Runnable{
	...
	EClientSocket clientSocket = new EClientSocket(this, m_signal);
	public void run(){
		
		try{
		clientSocket.eConnect("localhost", 4002, 0);
		
        final EReader reader = new EReader(clientSocket, m_signal);
			...
			public void run() {
				while (clientSocket.isConnected()) {
					...
					try {
						...
						e.printStackTrace();
//						mainL.error("Error in IB API Service",e);
					}
				}
			}
		}.start();
...
//		clientSocket.reqHistoricalData(1, getContract("AMD"), "", "1 hour", "1 min", "TRADES", 1, 1,true,null );
	}
	...
	private Contract getContract(String ticker){
